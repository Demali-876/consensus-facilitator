...
if ('Err' in approveResult) {
  throw new Error(`icrc2_approve failed: ${JSON.stringify(approveResult.Err, (_k, v) => typeof v === 'bigint' ? v.toString() : v)}`)
}
...
if (!verifyBody.isValid) throw new Error(`Verify failed: ${verifyBody.invalidReason} — ${verifyBody.invalidMessage}`)
...
if (!settleBody.success) throw new Error(`Settle failed: ${settleBody.errorReason} — ${settleBody.errorMessage}`)
