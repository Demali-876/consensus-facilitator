...
const EVM_NETWORKS = ['eip155:1', 'eip155:8453', 'eip155:84532', 'eip155:11155111'] as Network[]
...
const facilitator = new x402Facilitator()
  ...
  .onAfterSettle(async (ctx) => {
    console.log(
      `[settle] tx=${ctx.result.transaction} network=${ctx.result.network} payer=${ctx.result.payer}`
    )
  })
...
if (evmKey) {
  ...
  const clients = {
    'eip155:1': createWalletClient({
      account,
      chain: mainnet,
      transport: http(process.env.ETH_MAINNET_RPC_URL),
    }).extend(publicActions),
    'eip155:8453': createWalletClient({
      account,
      chain: base,
      transport: http(process.env.BASE_RPC_URL),
    }).extend(publicActions),
    'eip155:84532': createWalletClient({
      account,
      chain: baseSepolia,
      transport: http(process.env.BASE_SEPOLIA_RPC_URL),
    }).extend(publicActions),
    'eip155:11155111': createWalletClient({
      account,
      chain: sepolia,
  ...
  const evmSigner = toFacilitatorEvmSigner({
    ...
    verifyTypedData: (a) =>
      defaultClient.verifyTypedData(a as Parameters<typeof defaultClient.verifyTypedData>[0]),
...
if (svmKey) {
  ...
  const svmSigner = toFacilitatorSvmSigner(account, {
    [SVM_MAINNET]: createSolanaRpc(
      solMainnet(process.env.SOLANA_MAINNET_RPC_URL ?? 'https://api.mainnet-beta.solana.com')
    ),
    [SVM_DEVNET]: createSolanaRpc(
...
try {
  ...
  app.log.info(
    `networks registered: ${supported.kinds.map((k: { network: string }) => k.network).join(', ')}`
  )
} catch (err) {
  app.log.error(err)
  process.exit(1)
