...
const DB_PATH = resolve(process.env.DB_PATH ?? './facilitator.db')
...
const stmtCountResources = db.prepare(
  `SELECT COUNT(*) as total FROM bazaar_resources`
)
...
export function dbRecordNonce(principal: string, ledgerId: string, nonce: number): void {
  stmtInsertNonce.run(`${principal}:${ledgerId}:${nonce}`, Date.now() + NONCE_TTL_MS)
